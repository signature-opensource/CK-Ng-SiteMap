...
import { WebPagePageComponentType } from '@local/ck-gen/CK/IO/SiteMap/WebPagePageComponentType';
...
export class SiteMapProvider {
  ...
  readonly #tree: WritableSignal<ITreeNode> = signal( {
    children: {},
    isNavigable: false,
    pageTitle: '',
    path: ''
  ...
  readonly #preferredPages: WritableSignal<{ pageTitle: string; path: string; }[]> = signal( [] );
  readonly #homePagePath: WritableSignal<string | undefined> = signal( undefined );

  readonly breadcrumbItems = this.#breadcrumbItems.asReadonly();
  readonly preferredPages = this.#preferredPages.asReadonly();
  ...
  constructor() {
    this.#router.events
      ...
      .subscribe( e => {
        this.#currentUrl.set( this.#parseUrl( e.urlAfterRedirects ) );
        this.#buildBreadcrumb( this.#tree(), this.#currentUrl() );
      } );
  ...
  async #loadSiteMapAsync(): Promise<void> {
    try {
      ...
      this.#homePagePath.set( siteMap?.pages.find( p => p.webPageId === siteMap.homePageId )?.path );
      this.#preferredPages.set( siteMap?.preferredPages.map( pageId => {
        const page = siteMap.pages.find( p => p.webPageId === pageId );
        if ( !page ) {
...
interface ITreeNode {
  ...
  // TODO: I would personally rename this into "componentType"
